...
from padinfo.core.find_monster import find_monster, calc_ratio_name, calc_ratio_modifier, find_monster_search, \
    findMonster3
...
class PadInfo(commands.Cog, IdTest):
    ...
    async def _do_id(self, ctx, query: str):
        ...
        monster = await findMonster3(dgcog, raw_query)
    ...
    async def collabscroll(self, ctx, *, query):
        dgcog = await self.get_dgcog()
        monster: "MonsterModel" = await findMonster3(dgcog, query)
        ...
        state = SeriesScrollViewState(original_author_id, SeriesScrollMenu.MENU_TYPE, raw_query, query, color,
                                      series_id, initial_min_indices, monster_list, monster_list, int(rarity), all_rarities,
