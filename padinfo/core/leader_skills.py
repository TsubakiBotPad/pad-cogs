...
def _format_ls_text(hp, atk, rcv, resist=0, combo=0, fua=0, mfua=0, te=0):
    resist_val = ' {}{}%'.format(get_emoji('shield_amount'), _format_number(100 * resist)) if resist else ''
...
async def leaderskill_query(dbcog, raw_query, author_id):
    ...
    left_m = await dbcog.find_monster(left_query, author_id)
    if right_query:
        right_m = await dbcog.find_monster(right_query, author_id)
    else:
        right_m = left_m
