...
class TwitterCog2(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.settings = Twitter2Settings('twitter2')

        self.twitter_config = (self.settings.bot_settings['akey'], 
                               self.settings.bot_settings['asecret'],
                               self.settings.bot_settings['otoken'], 
                               self.settings.bot_settings['osecret'])
        print(self.twitter_config)

#         self.channels = list() # channels to push updates to
        self.channel_ids = self.settings.bot_settings['channels']
        self.ntweets = 0
        self.stream = None
...
and starts following a user if one was set upon construction."""
    ...
    async def _info(self, ctx):
        await ctx.send(self.info(ctx.channel))
...
#         await self.follow(tuser, ctx.message.channel)
    ...
    async def _addchannel(self, ctx, twitter_user):
        ...
        if already_following:
            if ctx.channel.id in self.channel_ids[twitter_user]:
        ...
        self.channel_ids[twitter_user].append(ctx.channel.id)
        self.settings.bot_settings['channels'] = self.channel_ids
...
Returns False if the user does not exist, True otherwise."""
        ...
        if self.stream_thread:
            if src_channel:
                await src_channel.send("Disconnecting from twitter.")
            self.stream.disconnect()
            self.stream_thread.join()
    ...
    def tweet(self, data):
        self.bot.get_event_loop().create_task(self.tweetAsync(data))
    ...
    async def send_all(self, message, twitter_user):
        ...
        for chan_id in self.channel_ids[twitter_user]:
            ...
            await self.bot.get_channel(chan_id).send(message)
        return True
...
class Twitter2Settings(CogSettings):
